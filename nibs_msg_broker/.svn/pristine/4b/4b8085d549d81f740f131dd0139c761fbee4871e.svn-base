...
public class FilemngTest {
	public static void main(String[] args) throws SchedulerException {
        ...
        */
        ...
        {
	    	String jobKey = "ShinhanFilemngJob";
	    	JobDataMap jobData = new JobDataMap();
	    	jobData.put("applicationContext", new ClassPathXmlApplicationContext("classpath:filemng/spring/context-filemng.xml", "classpath:filemng/spring/context-filemng-shinhanJob.xml"));
	        JobDetail job = JobBuilder.newJob(ShinhanFilemngJob.class).withIdentity(jobKey, "SqlService").usingJobData(jobData).build();
	        Trigger trigger = TriggerBuilder.newTrigger().withIdentity(jobKey, "FilemngService").withSchedule(CronScheduleBuilder.cronSchedule("0/5 * * * * ?")).build();
	        scheduler.scheduleJob(job, trigger);
		}
